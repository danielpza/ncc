...
const relocateLoader = eval(
  'require(__dirname + "/loaders/relocate-loader.js")'
);
...
function ncc(
  ...
  const shebangMatch = fs
    .readFileSync(resolvedEntry)
    .toString()
    .match(shebangRegEx);
  ...
  try {
    ...
    if (tsconfig.resultType === "success") {
      tsconfigMatchPath = tsconfigPaths.createMatchPath(
  ...
  if (!watch) {
    ...
    return {
      ...
      handler(handler) {
        if (watchHandler) throw new Error("Watcher handler already provided.");
  ...
  async function finalizeHandler(stats) {
    ...
    if (map && sourceMapRegister) {
      ...
      assets[`sourcemap-register${registerExt}`] = {
        source: fs.readFileSync(
          join(__dirname, `sourcemap-register.js.cache.js`)
        ),
        permissions: defaultPermissions,
      };
    }
    ...
    if (code.indexOf('"__webpack_require__"') === -1) {
      ...
      if (code.indexOf("__nccwpck_require2_") !== -1) {
        ...
        for (let i = 9; i > 1; i--) {
          if (code.indexOf(`__nccwpck_require${i}_`) === -1) continue;
          if (i === 9)
            throw new Error(
              "9 levels of ncc build nesting reached, please post an issue to support this level of ncc build composition."
            );
          code = code.replace(
            new RegExp(`__nccwpck_require${i}_`, "g"),
            `__nccwpck_require${i + 1}_`
          );
        }
      }
...
function getFlatFiles(mfsData, output, getAssetMeta, tsconfig, curBase = "") {
  for (const path of Object.keys(mfsData)) {
    ...
    // directory
    if (item[""] === true)
      getFlatFiles(item, output, getAssetMeta, tsconfig, curPath);
    // file
    else if (!curPath.endsWith("/")) {
      const meta = getAssetMeta(curPath.slice(1)) || {};
      if (curPath.endsWith(".d.ts")) {
        const outDir = tsconfig.compilerOptions.outDir
          ? pathResolve(tsconfig.compilerOptions.outDir)
          : pathResolve("dist");
